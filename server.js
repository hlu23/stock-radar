...
function overlayLiveBoard(rows) {
  ...
  return rows.map((row) => {
    ...
    return {
      ...
      volDelta: add > 0 ? add : (row.volDelta ?? add),
      relVol,
...
function pickLiveQuoteBatch(rows) {
  ...
  const a = hot[yahooQuoteCursor % hotN];
  const b = hot[(yahooQuoteCursor * 5 + 3) % hotN];
  const c = rows[yahooQuoteCursor % rows.length];
  yahooQuoteCursor += 1;
  const seen = new Set();
...
function kickYahooLive(rows) {
  if (liveKickBusy || !rows?.length) return;
  const batch = pickLiveQuoteBatch(rows);
  if (!batch.length) return;
  liveKickBusy = true;
  mapLimit(batch, 3, async (row) => {
    try {
      const q = await yahooQuote(row.symbol);
      if (!q) return;
      const prev = liveBoard.get(row.symbol) || {};
...
function buildIntraday(chart1m, chart1d, quote, extraBars) {
  ...
  const tapeMin = minuteTape(meta.symbol, quoteVol, last.t);
...
async function livePack(symbol, bias, locked) {
  ...
  const session = tapeSessionNow();
  const liveLast = quote?.last ?? chartQ?.last;
  const merged = {
    last: liveLast,
    bid: quote?.bid,
    ask: quote?.ask,
  ...
  if (quote?.regularMarketPrice > 0 && (session === "POST" || session === "CLOSED")) {
    rememberRthClose(symbol, quote.regularMarketPrice);
  }
  const intraday = buildIntraday(chart1m, c1d, merged, []);
...
function lastMinuteStats(chart) {
  ...
  const volDelta = hasPrint ? print.v : null;
  ...
  const mapped = await mapLimit(rows, 16, async (row) => {
    try {
      ...
      return {
        ...
        volDelta: minute.volDelta ?? row.volDelta ?? sessionVolumeStats(row.volume || minute.volume, row.avgVol).volDelta,
...
async function yahooMoverBoard() {
  const packs = await Promise.all([
    yahooScreener("day_gainers", 75).catch(() => []),
    yahooScreener("most_actives", 75).catch(() => []),
    yahooScreener("small_cap_gainers", 50).catch(() => []),
  ]);
  const seen = new Set();
  const rows = [];
  for (const q of packs.flat()) {
    if (!isUsListed(q)) continue;
    const row = mapMover(q);
    if (!row?.symbol || seen.has(row.symbol)) continue;
    seen.add(row.symbol);
    rows.push({
      ...row,
      dayChangePct: row.changePct,
      turnover: row.price && row.volume ? row.price * row.volume : null,
    });
  }
  return rows;
}
...
async function usGainers(params) {
  ...
  const movers = await yahooMoverBoard().catch(() => []);
  const universe = movers.length ? movers : listed;
  const watch = pickWatch(universe.length ? universe : listed, q);
  ...
  try {
    ...
    if (url.pathname === "/api/gainers") {
      ...
      return json(res, 200, {
        ...
        source: "yahoo",
        items,
        disclaimer: "야후 파이낸스 급등·거래량 상위와 1분봉 기준입니다. 지연 시세이며 투자 권유가 아닙니다.",
      });
    }
